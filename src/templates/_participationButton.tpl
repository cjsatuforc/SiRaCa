...
<div class="siracaParticipationBox">
    ...
    <a href="{link controller='Participation' object=$race}{/link}" 
            class="button buttonPrimary {if !$__wcf->user->userID || !$__wcf->getSession()->getPermission('user.siraca.canParticipateRace')}disabled{/if}"><span class="icon icon16 fa-sign-in"></span> <span>{lang}siraca.participation.link{/lang}</span></a>
</div>